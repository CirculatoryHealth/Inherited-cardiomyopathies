...
pheno <- c("Outcomes", "Heart_Failure", "Cardiomyopathy", 
           ...
           "Chronic_ischaemic_heart_disease", "Angina", "Cardiovascular_Death",
           "All_cause_mortality")
dat <- read.delim("results/output/CVD_enrichment_fisher.tsv") %>%
  filter(CM %in% c("ACM", "DCM", "HCM")) %>% filter(Phenotype %in% pheno)
...
pdf("results/figures/Figure3-Forest_plot.pdf", width = 10 * pix, height = 8 * pix)
dat %>% group_by(CM) %>%
...
ggarrange(v1, v2, v3, labels = c("A) ACM", "B) DCM", "C) HCM"), ncol = 3,
          font.label = list(size = 3.5, color = "black", face = "bold", 
                            family = "Helvetica"))
ggsave("results/figures/Figure4-Venn.pdf", width = 17 * pix, height = 5 * pix)
...
p1.1 <- ggplot(new, 
  ...
  annotate("text", x = 2.5, y = 94, label = "p=0.03", size = 1) +
  scale_fill_manual(breaks = c("Controls", "ACM", "DCM", "HCM"),
                    values = c("#4cbd97", "#ffd167", "#168ab2", "#ef476f")) +
  my_theme() +
  theme(legend.position="none", text = element_text(size = 3),
...
ggarrange(p1, p1.1, p2, p2.1, p3, p4, labels = "AUTO", 
          font.label = list(size = 3.5, color = "black", face = "bold", 
                            family = "Helvetica"),
          ncol = 2, nrow = 3)
ggsave("results/figures/Figure6-CMR_box.pdf", width = 8 * pix, height = 12 * pix)
